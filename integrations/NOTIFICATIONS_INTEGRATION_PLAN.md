...
1. **Client routes (`/notifications/*`)** – Any authenticated user. Operates on `req.user.id`.
   - Authentication: JWT delivered via the cookie `flick_auth_token` (or `AUTH_TOKEN` env override). Issued by [auth.public.controller.ts](file:///media/a-dev/01DCF07F273A0960/my-files/projects/boilerplate_backend/src/auth/auth.public.controller.ts).
...
- **Authentication:** Required (Cookie: `flick_auth_token`)
...
- **`200 OK`**
  ...
  {
    "data": [
      /* Notification objects, same shape as endpoint 1 */
    ],
    "total": 142,
    "page": 1,
    "limit": 20
...
### 8. Update Notification Preferences _(query-string PATCH)_
...
- **`403 Forbidden`** — `channel_name` user id ≠ JWT user id (anti-snooping).
  ...
  {
    "statusCode": 403,
    "message": "You can only subscribe to your own notification channel"
  }
  ```
...
```ts
channel.bind('notification:new', (p) => {
  /* prepend to list, badge++ */
});
channel.bind('notification:read', (p) => {
  /* patch row in cache      */
});
channel.bind('notification:read_all', () => {
  /* mark all read in cache */
});
channel.bind('notification:count', (p) => {
  /* set badge = p.unreadCount */
});
channel.bind('notification:delete', (p) => {
  /* remove from cache        */
});
channel.bind('payment:status', (p) => {
  /* toast + open billing UI */
});

const broadcast = pusher.subscribe('broadcast');
broadcast.bind('notification:new', (p) => {
  /* show global banner */
});
```
...
- [ ] Ensure the `flick_auth_token` HttpOnly cookie is set by the auth flow (backend sets it on login).
